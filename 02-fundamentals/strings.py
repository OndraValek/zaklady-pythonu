...
date_input = "27. 02. 2007"
databazove_datum = prevod_data(date_input)
print("Databázová podoba:", databazove_datum)
...
def generate_identifiers(phrase, style='python'):
    phrase = phrase.replace(' ', '_')  # Nahrazení mezer podtržítky

    if style == 'python':
        # Identifikátor pro Python (lowercase s podtržítky)
        identifier = phrase.lower().replace(' ', '_')
    elif style == 'js':
        # Identifikátor pro camelCase v JavaScriptu
        parts = phrase.split()
        identifier = parts[0].lower() + ''.join(word.capitalize() for word in parts[1:])
    else:
        raise ValueError("Neznámý styl identifikátoru. Použijte 'python' nebo 'js'.")
...
python_id = generate_identifiers(phrase, 'python')
js_id = generate_identifiers(phrase, 'js')

print("Python identifikátor:", python_id)
print("JS identifikátor:", js_id)
